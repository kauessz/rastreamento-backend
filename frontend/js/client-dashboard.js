(() => {
  ...
  const API_BASE_URL_LOCAL = window.API_BASE_URL;   // use esta constante local
  const PAGE_SIZE = 10;
  ...
  async function apiGet(path, withAuth = true) {
    ...
    const resp = await fetch(`${API_BASE_URL_LOCAL}${path}`, { headers });
  ...
  function openReport(path, params) { const q = new URLSearchParams(params).toString(); window.open(`${API_BASE_URL_LOCAL}${path}?${q}`, '_blank'); }
