...
// Base da API (usa window/API_BASE_URL definido no HTML; se não houver, define aqui)
...
function debounce(func, delay = 500) {
  let t; return (...a) => { clearTimeout(t); t = setTimeout(() => func.apply(this, a), delay); };
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  uploadForm?.addEventListener('submit', async (ev) => {
    ...
    try {
      const resp = await fetch(`${API_BASE_URL}/api/operations/upload`, {
  ...
  async function fetchAndRenderAliases() {
    ...
    try {
      ...
      // mostra somente pendentes por padrão
  ...
  aliasesTableBody?.addEventListener('click', async (e) => {
    ...
    if (e.target.classList.contains('button-reassign')) {
      ...
      try {
        const resp = await fetch(`${API_BASE_URL}/api/embarcadores/aliases/${id}/reassign`, {
          method: 'PUT',
    ...
    if (e.target.classList.contains('button-delete-alias')) {
      ...
      try {
        const resp = await fetch(`${API_BASE_URL}/api/embarcadores/aliases/${id}`, {
          method: 'DELETE', headers: { Authorization: `Bearer ${currentToken}` }
        });
        const out = await resp.json();
  ...
  async function populateEmbarcadorFilter() {
    try {
      const resp = await fetch(`${API_BASE_URL}/api/embarcadores`, {
        headers: { Authorization: `Bearer ${currentToken}` }
      });
      ...
      // carrega aliases após termos o combo de mestres
      fetchAndRenderAliases();
    } catch (e) { console.error(e); }
  }
