(() => {
  ...
  firebase.auth().onIdTokenChanged(async (user)=>{
    ...
    try{
      ...
      setTimeout(async ()=>{
        ...
        try{ currentToken=await getFreshToken(u); document.getElementById('userEmail')?.textContent=u.email; initOnce(); }
        catch{ alert('Não consegui confirmar sua sessão. Faça login novamente.'); try{await firebase.auth().signOut();}catch{} window.location.href='login.html'; }
      },1200);
    }
  });
  ...
  function renderAliasesTable(aliases){
    ...
    aliases.forEach(a=>{ const tr=document.createElement('tr'); tr.dataset.aliasId=a.id;
